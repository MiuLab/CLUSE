...
class data_helper(object):
    def __init__(self, path, sense_dim, batch_size, context_window, lang):
        ...
        self.load_vocab(path, sense_dim)
        self.rnd = []
    ...
    def read_bcws(self, path):
        context_window = self.context_window
        test_score = []
        testLocation = []
        testData = []
        with open(path+'/bi_ratings.txt') as fp:
            for e, line in enumerate(fp):
                line = line.strip()
                if e % 5 == 0:
                    test_score.append(float(line))
                elif e % 5 == 1 and self.lang == 'ch':
                    testLocation.append(int(line))
                elif e % 5 == 3 and self.lang == 'en':
                    testLocation.append(int(line))
                elif e % 5 == 2 and self.lang == 'ch':
                    testData.append(map(int,line.split(' ')))
                elif e % 5 == 4 and self.lang == 'en':
                    testData.append(map(int,line.split(' ')))
        ...
        self.testData = np.asarray(testData)
        self.testLocation = testLocation
        self.test_score = test_score

    def load_vocab(self, path, sense_dim):
